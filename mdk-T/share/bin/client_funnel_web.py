...
class controller:
	...
	def callback_mics(self, msg):
		...
		#t = time.time_ns()
		data = np.array(msg.data, dtype='float32')
		rms = np.sqrt(np.mean(data[0:1000]**2)) * (1.0 / 32768.0)
		self.audio_level = np.max((self.audio_level, rms))
		#dt = time.time_ns() - t
		#self.n += 1
		#self.t += dt
		#if self.n == 50:
		#	self.n = 0
		#	print(self.t / 1000000, self.audio_level)
		#	self.t = 0
	...
	def loop(self):
		...
		while self.active and not rospy.core.is_shutdown():
			...
			if not self.package is None:
				...
				self.msg_funnel_web.tones = self.tones
				self.tones = []
				...
				self.touch = [0, 0]
				self.audio_level = 0
				self.img_ann = miro.msg.img_annotation()
